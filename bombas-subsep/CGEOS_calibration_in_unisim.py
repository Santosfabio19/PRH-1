...
#composition = [1]
#list_of_species = [list_of_species[1]]
#list_of_names = [list_of_names[1]]

#delta_v = [volumn_desviation[1]]
...
pinit = gc_eos_class(mixture, 500+273.15, 2e4, None, 2, -1, Aij, delta_v, 'liquid')

pinit2 = gc_eos_class(mixturev, 500+273.15, 2e4, None, 2, -1, Aij, delta_v, 'gas')

pinit3 = gc_eos_class(mixturel, 500+273.15, 2e4, None, 2, -1, Aij, delta_v, 'liquid')
...
y0 = [0] + [0.1]*2 +[0.3/20]*20 + [0.8]

for T in range(600,650,10):
    
    soma, y1, vap, liq, viable = pinit.test_dew_P(T+273.15, 1000, y0)
...
for P in range(40000,100000,1000):
    
    soma, y1, vap, liq, viable = pinit.test_bubble_T(-30+273.15, P, y0)
    xco2.append(vap.mixture.x[1])
    Pt.append(P)
    sumy.append(soma)
    lin.append(1)
