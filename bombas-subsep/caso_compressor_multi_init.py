...
while k <= nsim:
    ...
    comp_pump_2 = simu2.stream_states['pump_2_in']['x'][-1]
    ...
    T_pump_2 = simu2.stream_states['pump_2_in']['T'][-1]
    P_pump_2 = simu2.stream_states['pump_2_in']['P'][-1]
...
############################################################################################3
    ...
    res_pump_2 =c2.character_online(simu2.stream_states['pump_2_in']['MassFlow'][-1],ns[i][1],corrente_pump_2, Gi3)
    ...
    #arm_mass_flow_pump_1.append(simu2.Streams['pump_1_in'].MassFlow())
    #arm_mass_flow_pump_2.append(simu2.Streams['pump_2_in'].MassFlow())
    eta_values, pressure_values = organize_separately(arm_pump_1, arm_pump_2)
    simu2.new_eta_head(eta_values[i],pressure_values[i])
...
from caso_unisim import simu

def compare_stream_state(obj1, obj2, stream, state, label1="Obj1", label2="Obj2"):
    y_name = {'P': 'Pressure /bar', 'T': 'Temperature /°C', 'dot_n': 'Molar flow rate /(kmol/h)'}
    factor_name = {'P': 100, 'T': 1, 'dot_n': 1}

    y1 = obj1.stream_states[stream][state]
    y2 = obj2.stream_states[stream][state]
    time1 = [i/60 for i in range(len(y1))]
    time2 = [i/60 for i in range(len(y2))]

    fig = plt.figure(dpi=150)
    ax = fig.add_subplot(1, 1, 1)
    ax.plot(time1, [i/factor_name[state] for i in y1], 'r', label=label1)
    ax.plot(time2, [i/factor_name[state] for i in y2], 'black', label=label2)

    ax.set_ylabel(f"{stream} {y_name[state]}", fontsize=12)
    ax.set_title(f"Comparação: {stream} ({state})")
    ax.set_xlabel("Time /h", fontsize=12)
    ax.legend()
    fig.tight_layout()

    os.makedirs("figuras", exist_ok=True)
    filename = f"figuras/comp_{stream}_{state}.png"
...
compare_stream_state(simu, simu2, stream=name_streams[2], state="T", label1="Simulação ns ", label2="Simulação eta_pressure")
