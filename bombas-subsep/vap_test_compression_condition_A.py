...
mixture.setup_entropy_ig_fun()

pinit = gc_eos_class(mixture, 97.29+273.15, 2.223e4, None, 2, -1, Aij, volumn_desviation, 'liquid')

solver_init = solver_eos(pinit)

y0 = [1e-10]*2 + [0.3]*2 +[1e-10]*20 + [1]

x0 = [0.1]+ [1e-10] + [0.8]*2 +[0.3/20]*20 + [0.2]

phi, vap, liq = solver_init.evaluate_flash(pinit, y0, x0)
