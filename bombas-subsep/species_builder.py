...
from casadi import *
from numpy import exp, log, array, roots, sum
...
class Species:
    ...
    def set_thermal_liquid_conductivity(self,fun):
        self.liq_ther_cond_fun = fun
    ...
    def evaluate_thermal_conductivities_liquid(self, T): #  Perry's Chemical Engineers' Handbook 8th edition        
        return array(self.liq_ther_cond_fun(T))[0][0]
...
class Mixture:
    ...
    def __init__(self,list_of_species,dict_composition):
        ...
        sum_x = sum([value for value in dict_composition.values()])
    ...
    def thermal_condutivity_liquid(self,T):
        lambda_ = array([species.evaluate_thermal_conductivities_liquid(T) for species in self.list_of_species])
        return  (sum(array(self.x)*lambda_**-2))**-1/2
