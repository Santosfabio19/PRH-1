...
class solver_eos:
    ...
    def test_bubble(self,Tb,P,y0):
        ...
        fliq = liq.evaluate_fugacity_coef()*array(self.fluid.mixture.x)
        sum_y_0 = 4.5
        while abs(sum_y_0-sum(y)) > self.delta_fun and itz < 100:
        ...
        return f,sum_y_0,y[0],y[-1],y_init[0],y_init[-1],it
    ...
    def evaluate_flash(self, fluid, y0, x0, phi=.1):
    ...
    def plot_many_envelope(self,list_of_solvers,list_colors):
        ...
        for i, solver in enumerate(list_of_solvers):
            ...
            if len(solver.water_line_P)>0:
                p7, = plot(array(solver.water_line_T), array(solver.water_line_P)/100,'--'+list_colors[i])
        config_plot(ax)
        ax.set_ylabel('Pressure /bar', fontsize=12)
        ax.set_xlabel('Temperature /°C', fontsize=12)
        fig1.tight_layout()
